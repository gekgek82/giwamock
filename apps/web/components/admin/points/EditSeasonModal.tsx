...
interface EditSeasonModalProps {
  ...
  onClose: () => void;
  onSubmit: (id: number, data: UpdateSeasonRequest) => Promise<void>;
}
...
export function EditSeasonModal({
  ...
  // Pre-populate form when season changes
  useEffect(() => {
    if (!season) return;
    setName(season.name);
    setRewardType(season.rewardType);
    setDescription(season.description ?? "");
    setStartDate(toDateInputValue(season.startDate));
    setEndDate(toDateInputValue(season.endDate));

    if (season.rewardType === "DISTRIBUTION") {
      const lpWeight = season.weights?.find(
        (w: { sector: string; weight: string }) => w.sector === "LP"
      );
      setLpPercent(lpWeight ? Math.round(parseFloat(lpWeight.weight) * 100) : 60);
    }

    if (season.rewardType === "FIXED" && season.rewardConfig) {
      setFixedAmount(String(season.rewardConfig.fixedAmount ?? "100"));
      setTriggerType(String(season.rewardConfig.triggerType ?? "WALLET_CONNECT"));
    }
  ...
  const buildRequest = (): UpdateSeasonRequest => {
    const base: UpdateSeasonRequest = {
      name: name.trim(),
      rewardType,
  ...
  const handleConfirm = async () => {
    ...
    try {
      await onSubmit(season.id, buildRequest());
      onClose();
    } catch (err) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative bg-ds-background-200 border border-ds-gray-400 rounded-lg p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-sm font-semibold text-ds-gray-1000">Edit Season</h3>
              <p className="text-sm text-ds-gray-700 mt-1">
                Modify season configuration
              </p>
            </div>
            <button
              onClick={onClose}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to update this season?"
        detail={
