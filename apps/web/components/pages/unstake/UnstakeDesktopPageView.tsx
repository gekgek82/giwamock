...
import { UnstakeFlow } from "@/components/unstake/UnstakeFlow";
import {
  useTokenByAddress,
...
export function UnstakeDesktopPageView() {
  return (
    <Suspense fallback={<UnstakeLoading />}>
      <UnstakeContent />
    </Suspense>
  );
}
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        {tokenId ? <CLUnstake tokenId={tokenId} /> : <BasicUnstake />}
      </main>
      <Footer />
    </div>
  );
...
function BasicUnstake() {
  ...
  // DepositInfoCard shows the currently-staked slice (what's unstakeable here).
  const stakedToken0 =
    balanceRaw && balanceRaw > 0n
      ? (token0ForBalance * Number(stakedAmountRaw)) / Number(balanceRaw)
      : 0;
  const stakedToken1 =
    balanceRaw && balanceRaw > 0n
      ? (token1ForBalance * Number(stakedAmountRaw)) / Number(balanceRaw)
      : 0;
  ...
  const price1 = selectedPool
    ...
    : 0;
  const depositUsd = stakedToken0 * price0 + stakedToken1 * price1;
  const hasStake = stakedToken0 > 0 || stakedToken1 > 0;
    ...
    <>
      <div className="mb-4">
        <PageHeader
          ...
          description={t("unstake.description")}
        />
      </div>
      <div className="mb-6">
        <PoolInfoHeader
      ...
      {(hasStake || isLoadingBalance || isLoadingIntent) && (
        <div className="mb-6">
          <DepositInfoCard
            depositNumber={1}
            tags={[
            ...
            tokenInfo={`${selectedPool.token0.symbol} ${fmt(stakedToken0)} / ${selectedPool.token1.symbol} ${fmt(stakedToken1)}`}
            status="Deposited"
            usdValue={`~$${fmt(depositUsd)}`}
            isLoading={isLoadingBalance || isLoadingIntent}
          />
        </div>
      )}
      <UnstakeFlow
...
function CLUnstake({ tokenId }: { tokenId: string }) {
  const t = useTranslations();
  const searchParams = useSearchParams();
  ...
  const stakedRatio =
    nftLiquidity > 0n ? Number(stakedAmountRaw) / Number(nftLiquidity) : 0;
  const stakedToken0 = deposited ? deposited.token0Amount * stakedRatio : 0;
  const stakedToken1 = deposited ? deposited.token1Amount * stakedRatio : 0;
  const stakedUsd = deposited ? deposited.usdValue * stakedRatio : 0;
  const hasStake = stakedToken0 > 0 || stakedToken1 > 0;
  ...
  return (
    <>
      ...
      {(hasStake || isLoadingPosition || isLoadingDeposit || isLoadingIntent) && (
        <div className="mb-6">
          <DepositInfoCard
            ...
            tokenInfo={`${token0Info.symbol} ${fmt(stakedToken0)} / ${token1Info.symbol} ${fmt(stakedToken1)}`}
            status="Deposited"
            usdValue={`~$${fmt(stakedUsd)}`}
            isLoading={isLoadingPosition || isLoadingDeposit || isLoadingIntent}
          />
        </div>
      )}
      <UnstakeFlow
